...
const AdminMechanics = () => {
    ...
    () =>
      mechanics.filter((mechanic) => {
        ...
        return (
          ...
          String(mechanic.cnpj || "").includes(term) ||
          String(mechanic.email || "").toLowerCase().includes(term)
        );
      }),
    [mechanics, search]
  );
  ...
  const updateMechanicStatus = async (mechanic: any, status: string) => {
    try {
      setUpdatingStatusId(mechanic.id);
      setError(null);
      ...
      setMechanics((current) =>
        current.map((item) => (item.id === mechanic.id ? { ...item, status } : item))
      );
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      {loading ? (
        <div className="glass-card p-4 text-sm text-muted-foreground">Carregando mecânicos...</div>
      ) : null}
      ...
      {!loading && (
        <div className="glass-card max-w-full overflow-hidden">
                  ...
                  >
                    ...
                    <td className="p-4 min-w-[180px]">
                      <Select
                        ...
                        onValueChange={(value) => updateMechanicStatus(mechanic, value)}
                        disabled={updatingStatusId === mechanic.id}
                      >
                        <SelectTrigger className="h-10 bg-secondary border-border">
                          <SelectValue placeholder="Selecione" />
                        </SelectTrigger>
                        <SelectContent>
                          {mechanicStatusOptions.map((status) => (
                    ...
                    <td className="p-4 text-right space-x-2">
                      ...
                      <Button variant="destructive" onClick={() => remove(mechanic.id)}>
                        Excluir
                      </Button>
                    </td>
                  </tr>
          ...
          {!filtered.length ? (
            <div className="p-4 text-sm text-muted-foreground">Nenhum mecânico encontrado.</div>
          ) : null}
        </div>
      )}
          ...
          <div className="glass-card w-full max-w-2xl p-6 space-y-5" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-bold text-foreground">
                {editing ? "Editar mecânico" : "Novo mecânico"}
              </h2>
              <button
                onClick={() => setModalOpen(false)}
