...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        setError(null);
        const [vehicleRes, driverRes, fuelingRes, maintenanceRes, loanRes, notificationRes] = await Promise.all([
  ...
  const stats = useMemo(() => {
    ...
    const totalLoanAmount = loans.reduce((sum, item) => sum + Number(item.amount || 0), 0);

    return { activeVehicles, activeDrivers, totalFuelCost, pendingMaintenances, totalLoanAmount };
  }, [vehicles, drivers, fuelings, maintenances, loans]);
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
            {[
              ...
              { icon: FileText, label: "Valor em empréstimos", value: `R$ ${stats.totalLoanAmount.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}` },
            ].map((stat) => (
