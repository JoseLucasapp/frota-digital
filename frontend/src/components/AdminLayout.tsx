...
import {
  ...
  FileText, ArrowLeftRight, User, Settings, HardHat
} from "lucide-react";
...
const AdminLayout = ({ children }: AdminLayoutProps) => {
  ...
  return (
    <div className="min-h-screen bg-background flex">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-sidebar border-r border-sidebar-border fixed h-full z-30">
        ...
        <div className="p-4 border-t border-sidebar-border">
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-base text-sidebar-foreground hover:bg-sidebar-accent transition-all"
          >
      ...
      {/* Mobile overlay */}
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 bg-background/80 backdrop-blur-sm z-40" onClick={() => setSidebarOpen(false)} />
      )}

      {/* Mobile Sidebar */}
      <aside className={cn(
        "lg:hidden fixed inset-y-0 left-0 w-72 bg-sidebar border-r border-sidebar-border z-50 transition-transform duration-300",
        ...
        <div className="p-4 flex items-center justify-between border-b border-sidebar-border">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-bold text-sidebar-foreground">ADS Frotas</h1>
          </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "w-full flex items-center gap-3 px-4 py-3 rounded-xl text-base font-medium transition-all",
                  isActive ? "gradient-primary text-primary-foreground" : "text-sidebar-foreground hover:bg-sidebar-accent"
                )}
              >
                <item.icon className="w-5 h-5" />
                {item.label}
        ...
        <div className="p-4 border-t border-sidebar-border absolute bottom-0 w-full">
          <button onClick={() => navigate("/login")} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-base text-sidebar-foreground hover:bg-sidebar-accent">
            <LogOut className="w-5 h-5" />
            Sair
          </button>
        </div>
        ...
        <header className="sticky top-0 z-20 bg-background/80 backdrop-blur-xl border-b border-border px-4 lg:px-8 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button onClick={() => setNotifOpen(true)} className="relative text-muted-foreground hover:text-foreground transition-colors">
              ...
              <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full gradient-primary text-[10px] text-primary-foreground flex items-center justify-center font-bold">3</span>
            </button>
            <div className="relative" ref={profileRef}>
              <button
                onClick={() => setProfileOpen(!profileOpen)}
                className="w-9 h-9 rounded-full gradient-primary flex items-center justify-center text-sm font-bold text-primary-foreground hover:opacity-90 transition-opacity"
              >
                AD
              </button>
              {profileOpen && (
                  <div className="absolute right-0 mt-4 w-56 bg-background border rounded-xl shadow-2xl py-2 z-50">
                  <div className="px-4 py-3 border-b border-border">
                    <p className="font-semibold text-foreground">Administrador</p>
                    <p className="text-sm text-muted-foreground">admin@prefeitura.gov.br</p>
                  </div>
                  ...
                  <div className="border-t border-border mt-1">
                    <button onClick={() => navigate("/login")} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-destructive hover:bg-secondary transition-colors">
